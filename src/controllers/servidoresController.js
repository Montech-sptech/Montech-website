...
async function adicionarServidoresUsuario(req, res) {
  ...
  servidorModel
    .adicionarServidoresUsuario(usuarioId, idServidor)
    ...
    .catch(function (erro) {
      console.log(erro);
      console.log(
        "\nHouve um erro ao atualizar os servidores do usuário! Erro: ",
        erro.sqlMessage,
      );
...
async function removerServidorUsuario(req, res) {
  ...
  servidorModel
    .removerServidorUsuario(usuarioId, idServidor)
    .then(function (resultado) {
      res.json(resultado);
    })
    .catch(function (erro) {
      console.log(erro);
      console.log(
        "\nHouve um erro ao remover o servidor do usuário! Erro: ",
        erro.sqlMessage,
      );
      res.status(500).json(erro.sqlMessage);
    });
  ...
  if (!nomeServidor) {
    ...
    try {
      ...
      for (let componenteInfo of componentes) {
        let resultadoComponente = await servidorModel.buscarIdComponentePorNome(
          componenteInfo.nomeComponente,
        );

        if (!resultadoComponente || resultadoComponente.length === 0) {
          console.log(
            `Componente '${componenteInfo.nomeComponente}' não encontrado no cadastro, ignorando limite de alerta.`,
          );
        } else {
      ...
      console.log(erro);
      console.log(
        "\nHouve um erro ao cadastrar o servidor! Erro: ",
        erro.sqlMessage,
      );
